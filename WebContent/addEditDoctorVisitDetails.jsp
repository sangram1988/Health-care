...
function displayResponse(request)
{
	if(request.readyState==4)
	{
		...
		if(request.status == 200 || request.status == 304)
		{
			...
			document.getElementById('allergy').value=request.responseText;
		//	document.getElementById('allreyDiv').innerHTML ="<label> Medicine Allergy :"+ request.responseText+" </label>";
...
function validateForm(){
	...
	var allergyMed=document.getElementById('allergy').value.toUpperCase();
	var prescriptions=document.getElementById('prescriptions').value.toUpperCase();
	
	
	var allergyMedicines=allergyMed.split(",");
	...
	if(startTime>=endTime){
		msg+='End date time must be greater than start date.\n';
	}

	for(var arrLength=0;arrLength<allergyMedicines.length;arrLength++){
		if(prescriptions.indexOf(allergyMedicines)>=0){
			msg+='The medicine :'+allergyMedicines+' is allergic to the patient.';
		}
	}
...
function showDeleateData(rowCnt,phrmacyId,userId,startTm,endTm,title,desc,prescription){
...
<body background="./images/body_bg.jpg">
	...
	<div>
		...
		<form name="appointmentForm" action="AddEditDeleteVisitAction"
			method="post">
			...
			<table align="center">
				<tr>
					...
					<td width="170">Id :</td>
					<td width="200"><input type="text" id="id" name="id" onchange="displayAllergy();" /></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><textarea rows="5" cols="16" id="prescriptions"
							name="prescriptions"></textarea></td>
					<td>Medicine Allergy :</td>
					<td><textarea rows="5" cols="16" id="allergy"
							name="allergy" readonly="readonly"></textarea></td>
				</tr>
